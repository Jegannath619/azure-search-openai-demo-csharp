...
internal static class ServiceCollectionExtensions
{
	...
	internal static IServiceCollection AddAzureServices(this IServiceCollection services)
	{
		...
		services.AddSingleton<ISearchService, MongoDbService>(sp =>
		{
			var config = sp.GetRequiredService<IConfiguration>();
			var mongoSettings = MongoClientSettings.FromUrl(new MongoUrl(config["MongoUrl"]));
			mongoSettings.AllowInsecureTls = true;
			mongoSettings.WriteConcern = WriteConcern.WMajority;
			var searchClient = new MongoClient(mongoSettings);
			return new MongoDbService(searchClient);
		});
